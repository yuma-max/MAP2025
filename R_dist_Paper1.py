...
def plot_shell_fraction_quad_cmap(sim_num_list, snapshots_to_analyze, infopath = "/home/takeichi/MAP/Code_Yuma_2025/Code/Datafiles/Data100_new_edited_Z.csv"):
    ...
    for i, sim_num in enumerate(sim_num_list):
        ...
        output_dir = os.path.join(base_data_folder, "r_dist_plots2_3")
        ...
        for snapshot_str in snapshots_to_analyze:
            ...
            for item in halo_ids:
                try:
                    ...
                    halo_data = {'infall_time': infall_time, "infall_mass":infall_mass}
            ...
            norm_smass = get_norm([h['infall_mass'] for h in survived_halos])
            norm_dmass = get_norm([h['infall_mass'] for h in disrupted_halos])
            ...
            for halo in survived_halos:
                ...
                if norm_stime:
                    axes[0,i].plot(bin_centers, fraction, color=cmap_surv(norm_stime(halo['infall_time'])))
                if norm_smass:
                    axes[1,i].plot(bin_centers, fraction, color=cmap_surv(norm_smass(halo['infall_mass'])))
            # Disrupted 
            for halo in disrupted_halos:
                fraction = np.divide(halo['mass_in_bins'], total_mass_in_bins, out=np.zeros_like(total_mass_in_bins, dtype=float), where=total_mass_in_bins!=0)
                if norm_dtime:
                    axes[0,i].plot(bin_centers, fraction, color=cmap_disr(norm_dtime(halo['infall_time'])))
                if norm_dmass:
                    axes[1,i].plot(bin_centers, fraction, color=cmap_disr(norm_dmass(halo['infall_mass'])))

            # --- Setting Plots ---
            axes[0,i].set_xscale('log') 
            axes[0,i].set_ylim(0, 1.05)
            axes[0,i].set_xlabel("Radial Distance (kpc)", fontsize=14)
            axes[0,i].grid(True, which="both", ls="--", alpha=0.5)
            axes[0,0].set_ylabel("Fraction of Mass at each shell", fontsize=14)
            axes[1,i].set_xscale('log') 
            axes[1,i].set_ylim(0, 1.05)
            axes[1,i].set_xlabel("Radial Distance (kpc)", fontsize=14)
            axes[1,i].grid(True, which="both", ls="--", alpha=0.5)
            axes[1,0].set_ylabel("Fraction of Mass at each shell", fontsize=14)
            if sim_num == 329:
                name="Elena"
            if sim_num == 229:
                name="Sonia"
            if sim_num == 242:
                name="Ruth"
            axes[0,i].set_title(f"{name}, z=0, colored by infall time", fontsize=16)
            axes[1,i].set_title(f"{name}, z=0, colored by infall mass", fontsize=16)
            ...
            add_colorbars(axes[0,i], norm_stime, norm_dtime, cmap_surv, cmap_disr, r"Surviving, Infall Time (Gyr)", r"Disrupted, Infall Time (Gyr)")
            add_colorbars(axes[1,i], norm_smass, norm_dmass, cmap_surv, cmap_disr, r"Surviving, Infall Mass ($M_\odot$)", r"Disrupted, Infall Mass ($M_\odot$)")
